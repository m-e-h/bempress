...
class Hybrid_Chat {
    ...
    /**
     * Separator between stanza author and mesbempress.
     *
     * @since  3.0.0
     * @access public
     * @var    string
    ...
    public function __construct( $content ) {
        ...
        // Filters for the chat mesbempress.
    ...
    public function get_transcript() {
        ...
        foreach ( $this->stanzas as $stanza ) {
            ...
            $mesbempress = sprintf( '<div class="chat-text">%s</div>', apply_filters( 'hybrid_chat_text', $stanza['mesbempress'] ) );

            // Format the entire stanza.
            $output .= sprintf( '<div class="chat-stanza chat-row %s">%s %s</div>', sanitize_html_class( "chat-speaker-{$speaker_id}" ), $author, $mesbempress );
        }
     ...
     *
     * @todo   Figure out how to split between author/time/mesbempress.
     * @since  3.0.0
     * @access public
     * @param  string  $content
     * @return array
    ...
    protected function get_stanza( $content ) {
        ...
        // If a speaker is found, split into array with author/mesbempress.
        if ( preg_match( '/(?<!http|https)' . $this->separator . '/', $content  ) ) {

            list( $author, $mesbempress ) = explode( $this->separator, trim( $content ), 2 );

            return array( 'author' => $author, 'mesbempress' => $mesbempress );
        }

        // Return just the mesbempress.
        return array( 'author' => '', 'mesbempress' => $content ? $content : '' );
    }
